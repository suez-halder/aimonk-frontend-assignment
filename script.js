            ...
            children: [
                {
                    ...
                    children: [
                        { name: 'child1-child1-child1', data: 'c1-c1-c1 Hello world' },
                        { name: 'child1-child1-child2', data: 'c1-c1-c2 coffee' },
                        { name: 'child1-child1-child3', data: 'c1-c1-c3 latte' },
                    ],
                },
...
function updateExport() {
    ...
    exportButton.setAttribute('href', `data:text/plain;charset=utf-8,${encodeURIComponent(jsonString)}`);
}
...
function createTagElement(tag, parentTag) {
    ...
    if (tag !== tree && tag !== tree.children[0] && tag !== tree.children[1]) {
    ...
    tagHeader.querySelector('.collapse-icon').addEventListener('click', () => {
        ...
        tagHeader.querySelector('.collapse-icon').textContent = tagElement.classList.contains('collapsed') ? '>' : 'v';
    });
    ...
    tagHeader.querySelector('.add-child-btn').addEventListener('click', () => {
        ...
        tagElement.querySelector('.children-container').appendChild(newChildElement);
        updateExport();
    });
...
function addChild(parentTag) {
    ...
    const newChild = createNewChild(parentTag, newChildIndex);
...
function searchTag(query, parentTag = tree) {
    if (parentTag.name.includes(query) || (parentTag.data && parentTag.data.includes(query))) {
        return parentTag;
    }
    if (parentTag.children) {
...
function getParentTagByIndex(index, parentTag = tree) {
    ...
    for (const childTag of parentTag.children) {
        const foundParent = getParentTagByIndex(index - childTag.children.length - 1, childTag);
        if (foundParent) {
            return foundParent;
        }
    }
